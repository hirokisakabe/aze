...
`aze` is primarily a CLI (`aze serve`) that serves a Vite + React + TypeScript SPA over a lightweight Node server and edits a local Markdown directory through `/api/notes`. The CLI entry is `bin/aze.ts` (bundled to `dist-cli/aze.js`); the fs-driver SPA is built to `dist-fs/`; the `/api/notes` middleware lives in `vite-fs-notes-plugin.ts`. App source lives in `src/`, with the entry at `src/main.tsx` and browser-side modules organized by responsibility: UI components in `src/components/` (`app.tsx`, `markdown.tsx`, `sidebar.tsx`), domain logic in `src/lib/` (`data.ts`, `frontmatter.ts`), and persistence in `src/repository/` (`notes-repository.ts`, `fs-notes-repository.ts`, `db.ts`, `assets.ts`). Tests are colocated next to their source (e.g. `src/lib/data.test.ts`, `src/components/markdown.dom.test.tsx`, `src/server/fs-notes-handler.test.ts`); shared test helpers and the jsdom setup live in `src/test-support/`. Playwright tests live in `e2e/`. Build output (`dist-cli/`, `dist-fs/`, `dist/`) is generated and should not be edited directly.
